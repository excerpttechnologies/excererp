...
// 🔒 IP Restriction Middleware (NO WHITELIST)
const checkIPRestriction = async (req, res, next) => {
  ...
  if (isIPv6) {
    ...
    if (!allowedIPv6Subnet.test(cleanIP)) {
      console.log(`🚫 ACCESS DENIED for IPv6: ${cleanIP}`);
      console.log('==========================================\n');
      return res.status(403).json({
        success: false,
        error: "Access Denied",
    ...
    console.log(`✅ IPv6 authorized: ${cleanIP}`);
    console.log('==========================================\n');
    return next();
  }
