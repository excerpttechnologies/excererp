...
billingCategorySchema.index({ companyId: 1, financialYearStart: 1, financialYearEnd: 1 });
billingCategorySchema.index({ companyId: 1, isActive: 1 });

billingCategorySchema.pre('save', async function(next) {
  if (this.isModified('rangeStart') || this.isModified('rangeEnd') || this.isNew) {
    ...
    if (overlappingCategory) {
      const error = new Error(`Range overlaps with existing billing category "${overlappingCategory.categoryName}" in this financial year`);
...
billingCategorySchema.methods.generateBillingNumber = function() {
  if (this.currentSequence > this.rangeEnd) {
    throw new Error(`Billing range exhausted for category ${this.categoryName}`);
  }

  const fyStartYear = this.financialYearStart.getFullYear();
  const fyEndYear = this.financialYearEnd.getFullYear();
  const billingNumber = `${this.prefix}${fyStartYear}-${fyEndYear}/${this.currentSequence.toString().padStart(4, '0')}`;

  return billingNumber;
};

billingCategorySchema.methods.getNextSequence = async function() {
  if (this.currentSequence > this.rangeEnd) {
    throw new Error(`Billing range exhausted for category ${this.categoryName}`);
  }
...
billingCategorySchema.statics.getCurrentFinancialYearRange = function() {
  const now = new Date();
...
billingCategorySchema.statics.findActiveForGeneration = function(companyId, financialYearStart, financialYearEnd) {
  return this.find({
    companyId,
    financialYearStart,
    financialYearEnd,
